...
public class WordCountClient implements Bootable {
  ...
  public static void main(String[] args) {
    ...
    ActorSystem system = ActorSystem.create("ClientApplication", ConfigFactory
        .load().getConfig("WCMapReduceClientApp"));

    final ActorRef fileReadActor = system
        .actorOf(new Props(FileReadActor.class));
    ...
    ActorRef actor = system.actorOf(new Props(new UntypedActorFactory() {
      public UntypedActor create() {
        return new WordCountClientActor(remoteActor);
      }
    }));
